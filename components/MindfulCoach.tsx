...
import { Send, Sparkles, User, UserPlus, Info } from 'lucide-react';
...
const MindfulCoach: React.FC = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto h-[calc(100vh-160px)] flex flex-col space-y-4">
      <div className="bg-emerald-50 dark:bg-emerald-900/10 p-4 rounded-2xl flex items-center gap-3 border border-emerald-100 dark:border-emerald-800">
        <div className="w-10 h-10 rounded-full bg-emerald-600 flex items-center justify-center text-white shrink-0">
          <Sparkles size={20} />
        </div>
        <div>
          <h2 className="font-bold text-emerald-800 dark:text-emerald-400 transition-colors">Mindfulness Coach</h2>
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] p-5 rounded-[2rem] shadow-sm flex flex-col gap-2 transition-colors ${
              msg.role === 'user' 
                ? 'bg-stone-800 dark:bg-stone-700 text-white rounded-tr-none' 
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="bg-white dark:bg-stone-900 border border-stone-100 dark:border-stone-800 p-6 rounded-[2rem] rounded-tl-none shadow-sm flex items-center gap-2">
              <div className="w-2 h-2 bg-stone-300 dark:bg-stone-600 rounded-full animate-bounce"></div>
              <div className="w-2 h-2 bg-stone-300 dark:bg-stone-600 rounded-full animate-bounce delay-100"></div>
              <div className="w-2 h-2 bg-stone-300 dark:bg-stone-600 rounded-full animate-bounce delay-200"></div>
            </div>
          </div>
        )}
      </div>

      <div className="bg-white dark:bg-stone-900 p-4 rounded-3xl border border-stone-100 dark:border-stone-800 shadow-lg flex items-center gap-3 transition-colors">
        <input 
          type="text"
          value={input}
          ...
          className="flex-1 bg-transparent border-none focus:ring-0 text-stone-700 dark:text-stone-200 py-2 px-2"
        />
        <button 
          onClick={handleSend}
          disabled={isLoading || !input.trim()}
          className={`p-3 rounded-2xl transition-all ${
            input.trim() ? 'bg-emerald-600 text-white shadow-emerald-200 dark:shadow-none shadow-md' : 'bg-stone-100 dark:bg-stone-800 text-stone-400 dark:text-stone-600 cursor-not-allowed'
          }`}
        >
          <Send size={20} />
        </button>
      </div>
      <p className="text-center text-[10px] text-stone-400 dark:text-stone-500 uppercase font-bold tracking-widest">
